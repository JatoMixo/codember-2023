...
fn count_words(words: Vec<String>) -> HashMap<String, u32> {
    ...
    for word in words {
        word_count.entry(word)
            .and_modify(|count| *count += 1) // If key already exists, add 1 to it
            .or_insert(1); // Else create a new key with value 1
    }

    word_count
}

fn get_final_string(words: Vec<String>, word_count: HashMap<String, u32>) -> String {
    let mut final_string: String = String::new();

    for word in &words {
        if final_string.contains(word) {
            continue;
        }

        final_string += &format!("{}{}", *word, word_count.get(word).unwrap());
    }
